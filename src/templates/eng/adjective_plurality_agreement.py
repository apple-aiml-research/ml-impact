...
@register_template
class EngAdjectivePluralityAgreement(TemplateBase):
    """Generates data for noun-adjective agreement in English sentences."""

    def __init__(self):
        self.setup_common(
            template_name="adjective_plurality_agreement.j2", lang_code="eng"
        )
        ...
        self.mismatched_features = ["plurality"]

        # Cache noun data
        self.noun_cache = {
            "occupations": self.placeholders.get_by_features("occupations"),
    ...
    def _generate_generation(self, additional_instruction: str = "") -> List[Dict]:
        ...
        feature_combinations = [(x, y) for x in ["HUM", "NHUM"] for y in ["SG", "PL"]]
        example_id = 1
    ...
    def _create_correct_judgments(
        ...
        for correct_inflection in correct_inflections:
            ...
            adj_feature_filter_clean = {
                k: v for k, v in adj_feature_filter.items() if k != "animacy"
            }
            noun_feature_clean = {
                k: v for k, v in noun_features.items() if k != "animacy"
            }
    ...
    def _apply_correct_inflection_logic(self, features_dict: Dict) -> Dict:
        """
        Plurality of the noun and adjective should agree
        ...
        """
        adj_feature_fict = features_dict.copy()
        if "plurality" in adj_feature_fict:
            adj_feature_fict.pop("plurality")
        return adj_feature_fict
