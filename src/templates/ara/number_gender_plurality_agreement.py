...
@register_template
class AraNumberGenderPluralityAgreement(TemplateBase):
    """Generates data for number-noun agreement in Arabic sentences, focusing on gender and plurality."""

    def __init__(self):
        self.setup_common(
            template_name="number_gender_plurality_agreement.j2", lang_code="ara"
        )
    ...
    def _create_incorrect_judgments(
        ...
        if incorrect_number_data.empty:
            raise ValueError(
                f"There are no left inflected forms for {incorrect_number_data} with {correct_inflection_set}."
            )
    ...
    def _apply_correct_inflection_logic(self, features_dict: Dict) -> Dict:
        """
        Apply Arabic-specific noun inflection logic for plural cardinal numbers. If the
        number is between 3-10, its gender should contradict the gender of the following noun
        ...
        if features_dict["plurality"] == "PL":
            features_dict["gender"] = (
                "FEM" if features_dict["gender"] == "MASC" else "MASC"
            )
