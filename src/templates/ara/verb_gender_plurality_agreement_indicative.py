...
@register_template
class AraVerbGenderPluralityAgreementIndicative(TemplateBase):
    """Generate data for noun-verb (indicative) agreement in Arabic sentences based on gender and plurality."""
    ...
    def _generate_generation(self, additional_instruction: str = "") -> List[Dict]:
        ...
        for feature_combination in feature_combinations:
            ...
            noun_gender, noun_plurality = (
                verb_features["gender"],
                verb_features["plurality"],
            )  # verb agrees with noun
            ...
            for name_group in name_groups:
                name_text = (
                    self.conjunction.join(name_group).strip()
                ...
                for root_verb in self.base_indicative_verbs:
                    ...
                    if verb_feature_filter["aspect"] == "PFV":
                        aspect = "perfective"
                    elif verb_feature_filter["aspect"] == "IPFV":
                        aspect = "imperfective"
                    else:
                        aspect = ""
                    if aspect:
                        additional_instruction = (
                            f"The inflected verb should be in the {aspect} form."
                        )
