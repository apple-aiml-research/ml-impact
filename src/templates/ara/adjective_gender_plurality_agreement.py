...
@register_template
class AraAdjectiveGenderPluralityAgreement(TemplateBase):
    """Generates data for noun-adjective agreement in Arabic sentences based on gender, plurality, and animacy."""

    def __init__(self):
        self.setup_common(
            template_name="adjective_gender_plurality_agreement.j2", lang_code="ara"
        )

        self.features = ["gender", "plurality", "animacy"]
        self.additional_features = {"case": "NOM"}
        ...
        self.noun_cache = {
            "occupations": {
                "MASC": self.placeholders.get_by_features(
                    "occupations", **{"gender": "MASC"}
                ),
                "FEM": self.placeholders.get_by_features(
                    "occupations", **{"gender": "FEM"}
                ),
            },
            "objects": {
                "MASC": self.placeholders.get_by_features(
    ...
    def _apply_correct_inflection_logic(self, features_dict: Dict) -> Dict:
        """
        Apply Arabic-specific adjective inflection logic for inanimate plural nouns. If the noun is plural and inanimate,
        ...
        if features_dict["animacy"] == "NHUM" and features_dict["plurality"] == "PL":
            features_dict["gender"] = "FEM"
            features_dict["plurality"] = "SG"
