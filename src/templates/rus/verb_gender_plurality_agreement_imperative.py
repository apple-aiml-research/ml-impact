...
@register_template
class RusVerbGenderPluralityAgreementImperative(TemplateBase):
    """Generate data for noun-verb (imperative) agreement in Russian utterances based."""

    def __init__(self):
        self.setup_common(
            template_name="verb_gender_plurality_agreement_imperative.j2",
            lang_code="rus",
        )
        ...
        for feature_combination in feature_combinations:
            ...
            for noun_gender in ["FEM", "MASC"]:
                ...
                for name_group in name_groups:
                    ...
                    base_imperative_verbs = self.placeholders.get_by_features(
                        "verbs_imp", **self.additional_features
                    ).base_word.unique()
                    for root_verb in base_imperative_verbs:
                        verb_feature_filter = {**verb_features, "base_word": root_verb}
                        inflected_forms = self.get_inflected_forms(
                            "verbs_imp", verb_feature_filter
                        )

                        correct_sentence = self.sentence_template.render(
                            name=name_text, scenario="generation"
                        ).strip()
                        additional_instruction = "The inflected verb should be in the imperative form. Assume an informal setting."
        ...
        for feature_combination in feature_combinations:
            ...
            for noun_gender in ["FEM", "MASC"]:
                ...
                for name_group in name_groups:
                    ...
                    additional_instruction = "Assume an informal setting."
                    for root_verb in base_imperative_verbs:
                        (
                            correct_examples,
                            correct_inflections,
    ...
    def _create_correct_judgments(
        ...
        correct_inflections = self.get_inflected_forms("verbs_imp", verb_feature_filter)
