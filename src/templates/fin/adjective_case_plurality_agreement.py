...
@register_template
class FinAdjectiveCasePluralityAgreement(TemplateBase):
    """Generates data for noun-adjective agreement in Finnish sentences based on plurality, and case."""

    def __init__(self):
        self.setup_common(
            template_name="adjective_case_plurality_agreement.j2", lang_code="fin"
        )
        ...
        self.features = [
            ...
            "plurality",
            "case",
        ]  # animacy is used here to just get meaningful adjectives for the nouns
        self.additional_features = {}
        self.mismatched_features = ["case", "plurality"]

        # Cache noun data
        self.noun_cache = {
            "occupations2": self.placeholders.get_by_features("occupations2"),
            "objects": self.placeholders.get_by_features("objects"),
        }
    ...
    def _generate_generation(self, additional_instruction: str = "") -> List[Dict]:
        ...
        example_id = 1
        additional_instruction = "The adjective should be in the base form and should modify the noun that follows."

        for feature_combination in feature_combinations:
            noun_features = dict(zip(self.features, feature_combination))
            animacy = noun_features["animacy"]
            noun_placeholder = "occupations2" if animacy == "HUM" else "objects"

            noun_data = self.noun_cache[noun_placeholder]
            ...
            for base_noun in base_nouns:
                ...
                inflected_nouns = self.get_inflected_forms(
                    noun_placeholder, noun_feature_filter, ok_empty=True
                )
                noun_feature_filter_clean = {
                    k: v
                    for (k, v) in noun_feature_filter.items()
                    if k in self.mismatched_features
                }
                for inflected_noun in inflected_nouns:
                    for base_adjective in base_adjectives:
                        ...
                        adj_feature_filter_clean = {
                            k: v
                            for (k, v) in adj_feature_filter.items()
                            if k in self.mismatched_features
                        }
    ...
    def _generate_judgment(self, additional_instruction: str = "") -> List[Dict]:
        ...
        example_id = 1
        additional_instruction = "The adjective should modify the noun that follows."
        for feature_combination in feature_combinations:
            noun_features = dict(zip(self.features, feature_combination))
            animacy = noun_features["animacy"]
            noun_placeholder = "occupations2" if animacy == "HUM" else "objects"
    ...
    def _create_correct_judgments(
        ...
        examples = []
        adj_feature_filter = {
            k: v for (k, v) in noun_features.items() if k in self.mismatched_features
        }
        adj_feature_filter = {
            **adj_feature_filter,
            **self.additional_features,
        ...
        for correct_inflection in correct_inflections:
            ...
            noun_features_clean = {
                k: v
                for (k, v) in noun_features.items()
                if k in self.mismatched_features
            }
            adj_feature_filter_clean = {
                k: v
    ...
    def _apply_correct_inflection_logic(self, features_dict: Dict) -> Dict:
        """
        AIn Finnish, the casing and plurality of the noun and adjective should agree

        Args:
            features_dict (Dict): Feature dictionary

        Returns:
            Dict: feature dictionary with correct casing and plurality for adjectives.
        """
